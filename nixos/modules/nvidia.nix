{ pkgs, lib, inputs, ... }: {
  hardware.nvidia = {
    ...
    prime = {
      offload = let mkOptional = lib.mkOverride 500;
      in {
        enable = mkOptional true;
        enableOffloadCmd = mkOptional true;
      };
  ...
  specialisation = {
    ...
    high-performance.configuration = let mkAlmostForce = lib.mkOverride 102;
    in {
